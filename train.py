...
def train(train_loader, tnet, criterion, optimizer, epoch):
    ...
    for batch_idx, (data1, data2, data3) in enumerate(train_loader):
        ...
        losses.update(float(loss_triplet.item()), data1.size(0))
        accs.update(acc, data1.size(0))
        emb_norms.update(loss_embedd.item()/3.0, data1.size(0))
...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        ...
        self.sum += val * n
        self.count += n
        self.avg = self.sum / self.count*1.0

def accuracy(dista, distb):
    margin = 0.0
    pred = (dista - distb + margin).cpu().data
    # print("sum:", (pred>0).sum().numpy())
    # print(dista.size(0))
    return ((pred > 0).sum().numpy())/(dista.size(0)*1.00000)
